...
public class SkuAttrValueServiceImpl extends ServiceImpl<SkuAttrValueMapper, SkuAttrValueEntity> implements SkuAttrValueService {
    ...
    public List<SkuAttrValueEntity> querySearchSkuAttrValuesByCidAndSkuId(Long cid, Long skuId) {
        ...
        List<AttrEntity> attrEntities = this.attrMapper.selectList(
                new QueryWrapper<AttrEntity>().eq("search_type", 1).eq("category_id", cid));
    ...
    public List<SaleAttrValueVo> querySaleAttrValueBySpuId(Long spuId) {
        //查询spu下sku
        List<SkuEntity> skuEntities = this.skuMapper
                .selectList(new QueryWrapper<SkuEntity>().eq("spu_id", spuId));
        ...
        // 先把List<skuAttrValueEntity>处理成map<attrId,List<SkuAttrValueEntity>>
        Map<Long, List<SkuAttrValueEntity>> map = skuAttrValues.stream()
                .collect(Collectors.groupingBy(SkuAttrValueEntity::getAttrId));
        //创建一个Vo集合用于手机SaleAttrValueVo对象
        List<SaleAttrValueVo> attrValueVos = new ArrayList<>();
        // 遍历map每一个map中的元素转化成SaleAttrValueVo对象
        map.forEach((attrId,skuAttrValueEntities) ->{
            ...
            saleAttrValueVo.setAttrValues(skuAttrValueEntities.stream()
    ...
    public String querySaleAttrValuesMappingSkuIdBySpuId(Long spuId) {
        ...
        //[{sku_id=1, attr_values=黑色,8G,128G}, {sku_id=2, attr_values=256G,8G,白色}]
        //把上面的数据模型处理成下面这个结构
        // {'黑色,8G,128G':1,'白色,8G,256G':2}
        Map<String, Long> attrValuesMappingSkuIdMap =
                maps.stream().collect(Collectors.toMap(map -> (String)map.get("attr_values"),map -> (Long)map.get("sku_id")));
